...
export const getGeminiKeys = (customKeys?: string[]): string[] => {
  if (customKeys && customKeys.length > 0) return customKeys;
  const keysStr = process.env.GEMINI_API_KEYS || process.env.GEMINI_API_KEY || '';
  return keysStr.split(',').map(k => k.trim()).filter(Boolean);
};

export const getRotatingKey = (customKeys?: string[]): string => {
  const keys = getGeminiKeys(customKeys);
...
export const getGeminiClient = (customKeys?: string[]) => {
  return new GoogleGenAI({ apiKey: getRotatingKey(customKeys) });
};
...
export async function geminiGenerateContent(
  ...
  for (let attempt = 0; attempt < keys.length; attempt++) {
    const apiKey = getRotatingKey(customKeys);
    try {
...
export async function analyzeHtmlWithAI(
  ...
  imageBase64?: string,
  customKeys?: string[]
): Promise<{ issues: ScanIssue[]; tokensConsumed: number }> {
  try {
