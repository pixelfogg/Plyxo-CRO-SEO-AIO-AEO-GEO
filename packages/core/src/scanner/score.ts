...
export function calculateCategoryScore(category: string, issues: ScanIssue[] = []): number {
...
export function calculateAllScores(scan: { scores?: Record<string, number>; issues?: ScanIssue[] }): Record<string, number> {
  const rawScores = scan?.scores || { 
    seo: 90, 
    performance: 80, 
    accessibility: 85, 
    'best-practices': 90 
...
export function calculateOverallScore(scan: { scores?: Record<string, number>; issues?: ScanIssue[] }): number {
