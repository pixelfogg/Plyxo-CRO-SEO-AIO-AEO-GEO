...
export const PageSpeedAnalyzer: AnalyzerPlugin = {
  ...
  async analyze(context: ScanContext): Promise<Record<string, AnalyzerResult>> {
    const { url } = context;
    console.log(`[Lighthouse] Starting analysis for ${url}`);
    ...
    if (process.env.GOOGLE_PSI_API_KEY) {
      apiUrl += `&key=${process.env.GOOGLE_PSI_API_KEY}`;
    }
    ...
    try {
      ...
      for (const cat of Object.keys(results)) {
        ...
        for (const ref of categoryData.auditRefs) {
          ...
          // We only want actionable failed audits.
          // scoreDisplayMode: 'binary' (0 or 1), 'numeric' (0 to 1), 'informative', 'notApplicable'
          if (
            ...
            // Clean up description markdown (Lighthouse uses [text](url) format)
            ...
            const issue: ScanIssue = {
              category: cat === 'best-practices' ? 'security' : cat, // map best-practices to security/other in UI
              title,
