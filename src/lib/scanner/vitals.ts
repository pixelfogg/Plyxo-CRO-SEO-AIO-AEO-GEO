...
/**
 * Fetches REAL Core Web Vitals for a URL from the Google PageSpeed Insights
 * API. Prefers CrUX field data (real users) and falls back to Lighthouse lab
 * metrics. Returns `null` when vitals cannot be measured — callers must treat
 * null as "not available" rather than substituting fabricated numbers.
 */
export async function fetchCoreWebVitals(url: string): Promise<CoreWebVitals | null> {
  ...
  try {
    ...
    // If we couldn't obtain any real metric, report null instead of guessing.
