...
import { getProjects } from "../seo/actions"; // We can reuse the SEO actions for getting projects
import { Loader2, Key, FolderOpen } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-4 border-b border-[#e6dfd8] dark:border-[#2e2b27] pb-6">
        <div>
          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#efe9de] dark:bg-[#252320] text-[#5db872] text-[12px] font-mono mb-2">
            <RadialSpike size={14} />
          ...
          <p className="text-[15px] text-[#3d3d3a] dark:text-[#a09d96] mt-1">
            Select a project to track keyword positions, search volume, difficulty, and uncover new opportunities.
          </p>
        </div>
      </div>
      ...
      {projects.length === 0 ? (
        <Card className="flex flex-col items-center justify-center p-12 text-center border-dashed border-2">
          ...
          <p className="text-sm text-zinc-500 max-w-sm mt-2 mb-6">
            You need to create a project first before running Keyword Intelligence.
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project) => (
            <ProjectCard 
              key={project.id}
              project={project}
              actionLabel="View Keyword Gap"
              actionUrl={`/dashboard/keywords/${project.id}`}
